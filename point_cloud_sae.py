...
class SAETemplate(torch.nn.Module, ABC):
    ...
    def compute_adjoint_classification_entropy(self, eval_dataset:PointCloudDataset):
        cluster_assignments=self.adjoint_classify(eval_dataset)
        return eval_dataset.compute_entropy_of_clustering(cluster_assignments)
    ...
    def report_evaluation(self, train_loss, eval_dataset:PointCloudDataset, step_number="N/A", write_location=None):
        ...
        adjoint_entropy=self.compute_adjoint_classification_entropy(eval_dataset)
        if write_location:
            write_location.update(self, train_loss, test_loss, dead_features, entropy, adjoint_entropy)
        else:
    ...
    def adjoint_classify(self, dataset:PointCloudDataset):
        '''
            uses the decoder to find the classifications
        '''
        embedded_points=embed_point_cloud(dataset.points, anchors=self.anchor_points, scale_factor=self.cloud_scale_factor)
        decoder_point_activations=embedded_points @self.decoder.T
        cluster_assignments=decoder_point_activations.argmax(dim=1)
        return cluster_assignments
...
class TrainingLog:
    ...
    def __init__(self):
        ...
        self.entropies=[]
        self.adjoint_entropies=[]

    def update(self, sae:SAETemplate, train_loss, test_loss, dead_features, entropy, adjoint_entropy):
        self.num_data_seen.append(sae.num_data_trained_on)
        self.train_losses.append(train_loss)
        self.test_losses.append(test_loss)
        self.dead_feature_counts.append(dead_features)
        self.entropies.append(entropy)
        self.adjoint_entropies.append(adjoint_entropy)
    ...
    def export_to_tensors(self):
        ...
        adjoint_entropies=torch.tensor(self.adjoint_entropies)
        return num_data_seen, train_losses, test_losses, dead_feature_counts, entropies, adjoint_entropies